<div class="main-page">
    <div class="container-fluid">
        <div class="row page-title-div">
            <div class="col-md-12">
                <h2 class="title">Listagem de Galeria</h2>
                <p class="sub-title"></p>
            </div>
        </div>
                        ...
                        <div class="panel-body">                 
                            <div class="row">
                                <div class="form-group col-sm-12 col-md-8">
                                    <h4><?= $viewVar['paginacao']['quanGaleria'] ?> Registro<?= $viewVar['paginacao']['quanGaleria'] == 1 ? '' : 's' ?></h4>
                                    <a href="<?= LINK ?>galeria/cadastro" class="btn bg-gray btn-wide"><i class="fa fa-plus"></i>Cadastrar</a>
                                </div>
                            </div>
                            <div class="row">
                                <?php
                                foreach ($viewVar['galeria'] as $item) {
                                    ...
                                    <div class="col-md-3 col-xs-6 imagem-galeria">
                                        <img src="<?= IMAGEMSITE ?>galeria/<?= $item['imagem'] ?>" class="card-img-top p-10" style="width: 100%" alt="Galeria não encontrado">
                                        <div class="card-body">
          
                                            <div style="text-align: center; background-color: #ccc; width: 100%; padding: 5px;">
                                                <a href="<?= LINK ?>galeria/editar/<?= $item['id'] ?>" class="btn btn-primary">Editar</a>
                            ...
                            <nav class="text-right">
                                <ul class="pagination">
                                    ...
                                    ?>">
                                        <a <?php
                                        if ($pagina != 1) {
                                            echo 'href="' . LINK . 'galeria/listar/' . $viewVar['paginacao']['anterior'] . '/' . $viewVar["paginacao"]["busca"] . '"';
                                        }
                                        ?> aria-label="Previous">
                                    ...
                                    for ($i; $i < $fim; $i++) {
                                        ...
                                        ?>"><a href="<?= LINK ?>galeria/listar/<?= $i + 1 ?>/<?= $viewVar['paginacao']['busca'] ?>"><?= str_pad($i + 1, 2, "0", STR_PAD_LEFT); ?></a></li>
                                            <?php
                                        }
                                        ?>
                                    <li class="<?php
                                    ...
                                    ?>">
                                        ...
                                        if ($pagina != $total and $fim != 0) {
                                            echo 'href="' . LINK . 'galeria/listar/' . $viewVar['paginacao']['proxima'] . '/' . $viewVar["paginacao"]["busca"] . '"';
                                        }
                                        ?> aria-label="Next">
...
<script>
    function buscar() {
        ...
        window.location.replace($("#link").val() + 'galeria/listar/' + busca);
    }
</script>