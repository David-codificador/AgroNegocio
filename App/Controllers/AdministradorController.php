...
class AdministradorController extends Controller {
    ...
    public function uploadFoto() {
        ...
        if ($retorno) {
            ...
            if ($resultado) {
                ...
                $this->redirect('administrador/minhasInformacoes');
            ...
            if (strlen($usuario) < 4) {
                ...
                if ($existe) {
                    if ($existe['usuario'] == $usuario) {
                        $resposta = ['status' => 1, 'msg' => '<p style="color: #0ddc16">Usuário atual!</p>'];
        ...
        if (is_numeric($id) and is_numeric($status)) {
            ...
            if ($resposta) {
                ...
                $info = [
                    ...
                    'tabela' => \App\Models\Entidades\Administrador::TABELA['descricao'],
                    'descricao' => 'O ' . Sessao::getAdministrador('tipo') . ' ' . Sessao::getAdministrador("nome") . ', efetuou a exclusão do administrador ' . $administrador['nome']
                ];
        ...
        if (is_numeric($id)) {
            ...
            if ($resultado == FALSE) {
                ...
                $info = [
                    ...
                    'campos' => $x,
                    'tabela' => \App\Models\Entidades\Administrador::TABELA['descricao'],
                    'descricao' => 'O ' . Sessao::getAdministrador('tipo') . ' ' . Sessao::getAdministrador("nome") . ', efetuou a edição da administrador "' . $resultado['nome'] . '".'
                ];
    ...
    public function minhasInformacoes($param) {
        $this->validaAdministrador();
        ...
        $this->render("administrador/minhasInformacoes", Sessao::getAdministrador('nome'), $css, $js, 1);
    }

    public function salvarMinhasInformacoes() {
        $this->validaAdministrador();
        ...
        $dados['nome'] = $vetor['nome'];
        $dados['telefone'] = $vetor['telefone'];
        $dados['email'] = $vetor['email'];
        $dados['usuario'] = $vetor['usuario'];

        if ($dados['senha'] != '') {
            $dados['senha'] = $vetor['senha'];
        } else {
            unset($dados['senha']);
        }

        $resultado = $bo->editar(\App\Models\Entidades\Administrador::TABELA['nome'], $dados, "id = ?", [Sessao::getAdministrador('id')], 1, \App\Models\Entidades\Administrador::CAMPOSINFO);

        if ($resultado == FALSE) {
            if (!Sessao::existeMensagem()) {
                Sessao::gravaMensagem("Ops, não conseguimos editar suas informações", 2);
            }
        } else {
            unset($dados['senha']);
            ...
            $info = [
                ...
                'campos' => $x,
                'tabela' => \App\Models\Entidades\Administrador::TABELA['descricao'],
                'descricao' => 'O ' . Sessao::getAdministrador('tipo') . ' ' . $vetor["nome"] . ', editou suas informações.'
            ];
            ...
            Sessao::gravaMensagem(Sessao::getAdministrador('nome') . "as suas Informações foram editadas.", "Sucesso", 1);
            $this->redirect('administrador/minhasInformacoes');
        }
    ...
    function atualizarDadosSessao() {
        $bo = new \App\Models\BO\AdministradorBO;
        $administrador = $bo->selecionarVetor(\App\Models\Entidades\Administrador::TABELA['nome'] . ' a inner join ' . \App\Models\Entidades\TipoAdministrador::TABELA['nome'] . ' ta on a.tipo_administrador_id = ta.id ', ['a.*', 'ta.tipo'], "a.id = '?'", [Sessao::getAdministrador('id')], '');
        ...
        if ($administrador) {
            ...
            Sessao::gravaMensagem("Usuário não esta logado.", "Erro!", 2);
            $this->redirect('administrador/acesso');
        }
    }
